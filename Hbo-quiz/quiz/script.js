...
function downloadRapport() {
  ...
  // Make rapport-page visible so html2canvas can capture it.
  // It renders above the quiz content briefly; the loading state on the
  // button signals to the user that generation is in progress.
  var rapportEl = document.getElementById('rapport-page');
  rapportEl.style.cssText =
    'display:block; background:#fff; padding:24px; ' +
    'box-sizing:border-box; width:794px; max-width:100%;';

  // Plain filename — no HTML entities
  var safeName = (studentFirstName + ' ' + studentLastName)
    .replace(/[^a-zA-Z0-9 _\-]/g, '')
    .trim()
    .replace(/\s+/g, '_');
  var fileName = 'HBO-ICT_Rapport_' + safeName + '.pdf';
  ...
  var btn = document.querySelector('.res-download');
  if (btn) { btn.disabled = true; btn.textContent = 'Bezig met genereren\u2026'; }

  function resetBtn() {
    rapportEl.style.cssText = 'display:none;';
    if (btn) { btn.disabled = false; btn.innerHTML = '&#8659; Download jouw rapport (PDF)'; }
  }

  html2pdf().set(opt).from(rapportEl).save()
    .then(resetBtn)
    .catch(function(err) { console.error('PDF fout:', err); resetBtn(); });
}
